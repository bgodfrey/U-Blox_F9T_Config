...
def initial_probe(ser, verbose=False):
    ...
    if verbose:
        def _clean(b): return b.strip("\x00") #b.decode(errors="ignore").strip("\x00")
...
def parse_args():
    ...
    # Serial overrides (useful with --auto-uid, or if you want to override the manifest)
    ap.add_argument("--port", help="Serial port override (e.g. /dev/ttyACM0)")
    ap.add_argument("--baud", type=int, help="Serial baud override")
...
def _norm_uid(uid: str) -> str:
    """Normalize a SEC-UNIQID string to uppercase hex without separators."""
    s = (uid or "").strip().upper()
    return re.sub(r"[^0-9A-F]", "", s)


def _is_manifest(doc: dict) -> bool:
    """Heuristic: your new format has top-level keys: global, role, devices."""
...
def _list_manifest_devices(doc: dict) -> List[Tuple[str, str, str]]:
    """Return [(uid, alias, role), ...] from a manifest."""
...
def _select_manifest_device(doc: dict, *, uid: Optional[str], alias: Optional[str]) -> Tuple[str, Dict[str, Any]]:
    """Select a device entry from a manifest by uid or alias."""
    devs = doc.get("devices", {}) or {}
    if not devs:
    ...
    # No selector provided: if only one device, pick it.
    if len(devs) == 1:
        k = next(iter(devs.keys()))
        return _norm_uid(k), (devs[k] or {})

    # Otherwise ask caller to specify.
...
def _build_plan_from_manifest(doc: dict, args: argparse.Namespace) -> Dict[str, Any]:
    """Flatten manifest (global + role + device) into the old 'flat' schema."""
    g = doc.get("global", {}) or {}
    ...
    # Register CSV is optional in the manifest; keep compatibility with your old flat format
    ...
    # De-duplicate by key (later entries override earlier ones), while keeping a
    # stable order close to the user's intent.
    dedup: Dict[str, Any] = {}
    order: List[str] = []
    ...
    # Position: for F9T fixed-mode configuration
...
def main():
    ...
    # --- Support both legacy "flat" config and the newer manifest format ---
    if _is_manifest(doc):
    ...
    verify_layer = (args.verify_layer or doc.get("verify_layer", "RAM")).upper()  # NOTE: FLASH may NAK VALGET
    ...
    with serial.Serial(port, baudrate=baud, timeout=0.5) as ser:
        ...
        # Prefer auto-detect (MON-VER), but fall back if something odd happens.
        try:
            model = detect_model(ser)
        except Exception:
            model = 'UNKNOWN'
